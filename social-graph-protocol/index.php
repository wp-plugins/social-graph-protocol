...
EOD;
		...
		function add_stylesheet( $hook ) {
			...
			if( 'toplevel_page_open-graph-protocol/index' != $hook ) return;
			
		    echo "<link rel='stylesheet' href='" . WP_PLUGIN_URL . "/open-graph-protocol/css/open-graph-menu.css' type='text/css' media='all' />";
		}
		...
		function create_menu() {
			...
			$open_graph_hook = add_menu_page ( __ ( 'Open Graph', 'Facebook Open Graph Protocol' ), __ ( 'Open Graph', 'Facebook Open Graph Protocol' ), 8, 
		...
		function open_graph_menu() {
			...
			if( isset( $_POST['save_fb_open_graph_settings'] ) ) {
				...
				if( !empty( $_POST['custom_post_types'] ) ) update_option( 'custom_post_types', trim( $_POST['custom_post_types'] ) ); else delete_option( 'custom_post_types' );
			...
			include_once( ABSPATH . 'wp-content/plugins/open-graph-protocol/includes/open-graph-menu.php' );
		...
		function custom_meta_boxes() {
			$custom_post_types = explode( ',', get_option( 'custom_post_types' ) ); // An array of custom post types
		...
		function render_meta_box_content( $post ) {
			...
			include_once( ABSPATH . 'wp-content/plugins/open-graph-protocol/includes/meta-box.php' );
