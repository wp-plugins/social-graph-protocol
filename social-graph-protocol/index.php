<?php
	...
	*/
	...
	// add_action( 'init', array( 'fbOpenGraphByBizzyLabs', 'load_textdomain' ) ); /* Removed Temporarily in 1.1.4 */
	...
	class fbOpenGraphByBizzyLabs {
		...
		function add_help_documentation() {
			...
			$screen->add_help_tab( array(
				'id'      => 'site-description',
				'title'   => __('Site Description', 'social-graph-protocol'),
				'content' => __('The first option to set is pretty straight forward, its a one to two sentence description of your overall website and its limited to 255 characters. You can enter 
								 anything you want, however we highly suggest you write something meaningful about your website. Additionally if you leave this value blank, it will attempt to 
								 default to your websites Tagline specified under the Wordpress Settings menu. If the Tagline value is not set and you do not provide a description, then this meta 
								 tag will not be generated in your html.', 'social-graph-protocol'),
			));
			
			$screen->add_help_tab( array(
				'id'      => 'site-image',
				'title'   => __('Site Image', 'social-graph-protocol'),
				'content' => __('The second option asks you to provide a URL to a screenshot of your website, as an example if I wanted to use the bizzyLabs logo as the screenshot I would insert 
								 the text http://www.bizzylabs.com/wp-content/uploads/2011/08/logo.png. One thing to note is that the image must be at least 50px by 50px and have a maximum aspect 
								 ratio of 3:1.  Facebook supports PNG, JPEG and GIF formats only.', 'social-graph-protocol'),
			));
			
			$screen->add_help_tab( array(
				'id'      => 'facebook-uid',
				'title'   => __('Faceboook UID(s)', 'social-graph-protocol'),
				'content' => __('The third option asks you to enter your Facebook user id or a comma-separated list of either user IDs and/or Facebook platform application IDs that will be used to 
								 administer your page. It is valid to include both id types. Basically only list the IDs of people that you want to give admin rights to, if this is only you then 
								 only enter in your Facebook UID and no one elses. Now if you have absolutely no clue how to obtain your Facebook UID, you can visit the
								 <a href="http://www.facebook.com/insights/" title="Facebook Insights" target="_blank">Facebook Insights</a> page, click the green Insights for your website button 
								 near the upper right, select your name from the "Link With" drop down and just below that you\'ll notice a meta tag simply copy that meta tag it will look 
								 something like &lt;meta property="fb:admins" content="<span style="background-color:#fffc00;">655660025</span>" /&gt; where the highlighted part is going to be 
								 your Facebook UID that you include on the settings page.', 'social-graph-protocol'),
			));
		...
		function add_stylesheet( $hook ) {
			...
			if( 'toplevel_page_social-graph-protocol/index' == $hook  ) {
			...
			elseif( 'post.php' == $hook || 'post-new.php' == $hook ) {
				wp_enqueue_script( 'blCollapse', plugins_url('/js/collapse.js', __FILE__) );
				echo "<link rel='stylesheet' href='" . WP_PLUGIN_URL . "/social-graph-protocol/css/open-graph-menu.css' type='text/css' media='all' />";
			}
		}
		...
		function create_menu() {
			...
			add_action( 'load-' . $open_graph_hook, array( 'fbOpenGraphByBizzyLabs', 'add_help_documentation' ) );
		}
