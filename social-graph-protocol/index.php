<?php
	...
	*/
	...
	add_action( 'init', array( 'fbOpenGraphByBizzyLabs', 'load_textdomain' ) );
	...
	class fbOpenGraphByBizzyLabs {
		...
		function load_textdomain() {
			load_plugin_textdomain( 'social-graph-protocol', false, 'social-graph-protocol/i18n' );
		}
		...
		function add_help_documentation( $contextual_help, $screen_id, $screen ) {
			...
			if( $screen_id == $open_graph_hook ) {
				$contextual_help = '<p style="line-height:20px;font-size:13px;">' . 
					__('The Social Graph Protocol plugin contains four settings that should be set in order to utilize the full potential of the plugin. We ask you to set ', 'social-graph-protocol') . 
					__('these values because they are the values that are going to be used for your homepage, these values will', 'social-graph-protocol') . 
					' <span style="border-bottom: 3px double;">' . __('NOT', 'social-graph-protocol') . '</span> ' . 
					__('carry over to individual Wordpress posts or pages, they are strictly for the homepage of your website, you\'ll be able to set the values of each ', 'social-graph-protocol') .
					__('page or post on the individual edit screens.', 'social-graph-protocol') . '</p>';
					
				$contextual_help .= '<p style="line-height:20px;font-size:13px;">' . 
					__('The first option to set is pretty straight forward, its a one to two sentence description of your overall website and its limited to', 'social-graph-protocol') . ' 255 ' . 
					__('characters. You can enter anything you want, however we highly suggest you write something meaningful about your website. Additionally ', 'social-graph-protocol') . 
					__('if you leave this value blank, it will attempt to default to your websites Tagline specified under the Wordpress Settings menu. If the ', 'social-graph-protocol') . 
					__('Tagline value is not set and you do not provide a description, then this meta tag will not be generated in your html.', 'social-graph-protocol') . '</p>';
					
				$contextual_help .= '<p style="line-height:20px;font-size:13px;">' . 
					__('The second option asks you to provide a URL to a screenshot of your website, as an example if I wanted to use the', 'social-graph-protocol') . ' bizzyLabs ' . 
					__('logo as the screenshot I would insert the text', 'social-graph-protocol') . 
					' http://www.bizzylabs.com/wp-content/uploads/2011/08/logo.png. ' . __('One thing to note is that the image must be at least', 'social-graph-protocol') . ' 50px ' . 
					__('by', 'social-graph-protocol') . ' 50px ' . __('and have a maximum aspect ratio of', 'social-graph-protocol'). ' 3:1 ' . __('Facebook supports', 'social-graph-protocol') . ' PNG, JPEG ' . 
					__('and', 'social-graph-protocol') . ' GIF ' . __('formats only.', 'social-graph-protocol') . '</p>';
					
				$contextual_help .= '<p style="line-height:20px;font-size:13px;">' . 
					__('The third option asks you to enter your Facebook user id or a comma-separated list of either user IDs and/or Facebook platform application IDs ', 'social-graph-protocol') . 
					__('that will be used to administer your page. It is valid to include both id types. Basically only list the IDs of people that you want to give admin ', 'social-graph-protocol') . 
					__('rights to, if this is only you then only enter in your Facebook UID and no one elses. Now if you have absolutely no clue how to obtain your Facebook UID, you can visit the', 'social-graph-protocol') . 
					' <a href="http://www.facebook.com/insights/" title="Facebook Insights" target="_blank">' . 
					__('Facebook Insights', 'social-graph-protocol') . '</a> ' . 
					__('page, click the green Insights for your website button near the upper right, select your name from the "Link With" drop down and just below that you\'ll notice a meta tag simply copy that meta tag it will look something like', 'social-graph-protocol') . 
					' &lt;meta property="fb:admins" content="<span style="background-color:#fffc00;">655660025</span>" /&gt; ' . __('where the highlighted part is going to be your Facebook UID that you include on the settings page.', 'social-graph-protocol') . '</p>';
					
				$contextual_help .= '<p style="line-height:20px;font-size:13px;">' . 
					__('Finally the very last option is a simple drop down menu with two options, one option is called Blog and the second option is called website. ', 'social-graph-protocol') . 
					__('If you think your website is more of a blog then choose blog, but if you think your website is more of a traditional website then select ', 'social-graph-protocol') . 
					__('website. The difference is minor and a large majority of Wordpress users will select blog. The difference is that a Website is considered ', 'social-graph-protocol') . 
					__('to be more static content as where a blog your continually writing new content making it more dynamic. Overall its a very stupid feature in ', 'social-graph-protocol') . 
					__('the Facebook Open Graph and I\'m sure they\'ll end up changing it or clarifying the difference a bit more in the future.', 'social-graph-protocol') . '</p>';
			}
		...
		function create_menu() {
			...
			$open_graph_hook = add_menu_page ( __( 'Social Graph', 'social-graph-protocol' ), __ ( 'Social Graph', 'social-graph-protocol' ), 8, 
			__FILE__, array( 'fbOpenGraphByBizzyLabs', 'open_graph_menu' ) );
			
			// Add a sub-menu called Add Videos
			add_submenu_page( __FILE__, __( 'Settings', 'social-graph-protocol' ), __( 'Settings', 'social-graph-protocol' ), 8, __FILE__, array( 'fbOpenGraphByBizzyLabs', 'open_graph_menu' ) );
		}
		...
		function open_graph_menu() {
			...
			if( isset( $_POST['save_fb_open_graph_settings'] ) ) {
				...
				$message = __( 'Your Settings Have Been Successfully Saved.', 'social-graph-protocol' );
			}
		...
		function custom_meta_boxes() {
			...
			add_meta_box( 
		        ...
		        __( 'Social Graph Protocol Options', 'social-graph-protocol' ),
		...
		function add_meta_html() {
			...
			$html = "\n<!-- " . __( 'Start Of Facebook Open Graph Meta Data', 'social-graph-protocol' ) . " -->\n";
			...
			$html .= '<!-- ' . __( 'End Of Facebook Open Graph Meta Data', 'social-graph-protocol' ) . " -->\n"; // End the HTML output with this little hidden tag
